"use strict";
/*----------------------------------------后台Api地址----------------------------------------*/
exports.APP_SERVE_URL = '/app'; //'http://read.jfapps.com/app';
/*----------------------------------------文件服务器地址----------------------------------------*/
exports.FILE_SERVE_URL = ''; //文件服务:测试环境
/*----------------------------------------app版本升级服务地址----------------------------------------*/
exports.APP_VERSION_SERVE_URL = ''; //app版本升级服务;测试环境,查询app最新版本号,更新日志等信息.
exports.APP_VERSION = '1.0'; //app版本升级服务;测试环境,查询app最新版本号,更新日志等信息.
exports.APP_NAME = '花生LK'; //app版本升级服务;测试环境,查询app最新版本号,更新日志等信息.
exports.IS_DEBUG = true; //是否开发(调试)模式
exports.DEFAULT_AVATAR = './assets/imgs/logo@2x.png'; //用户默认头像
exports.LOGO = './assets/imgs/logo@2x.png'; //用户默认头像
exports.IMAGE_SIZE = 1024; //拍照/从相册选择照片压缩大小
exports.QUALITY_SIZE = 94; //图像压缩质量，范围为0 - 100
exports.REQUEST_TIMEOUT = 12000; //请求超时时间,单位为毫秒
//# sourceMappingURL=constants.js.map